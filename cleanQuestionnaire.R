# Last modified by Hause Lin 08-05-17 5:02 PM
# Function to reverse-code questionnaires (either long or short form data).
...
# form: form of your data (wide or long; default = 'wide')
...
cleanQuestionnaire <- function(data, subjectCol = 1, scaleName, scaleMin = 1, scaleMax = 7, form = 'wide', subscales = list(), itemsToReverse = c(), checkReliability = TRUE) {
    ...
    #### process data differently depending on wide or long form ####
    ...
    if (form == 'wide') {
        ...
        dataLong <- data
        if (grep('rating', colnames(dataLong))) {
            n <- grep('rating', colnames(dataLong))
            colnames(dataLong)[n] <- 'score'
        }
        if (grep('qNo', colnames(dataLong))) {
            n <- grep('qNo', colnames(dataLong))
            colnames(dataLong)[n] <- 'item'
        }
        if (grep('question', colnames(dataLong))) {
            n <- grep('question', colnames(dataLong))
            dataLong <- select(dataLong, -n)
        } 
        
        scaleN <- dataLong[, .N, by = pNo][1, N]
        scaleItems <- dataLong[1:scaleN, as.character(item)]
        ...
        checkScaleItems <- tbl_dt(data_frame(scale = scaleName, itemName = scaleItems, item = as.numeric(1:length(scaleItems)), toReverse = 'no'))
        checkScaleItems[item %in% itemsToReverse, toReverse := 'yes']
        
        
    } else {
        stop('Wrong format. Long or wide form input data?')
    }
    ...
    confirm <- readline("Press 1 to proceed or 0 to stop. If you press 1, a list with wide and long forms of the summary data (data.table) will be generated. ")
    if (confirm == '0') {
    ...
    if (checkReliability) {
        ...
        if (length(subscales) > 0) { # if there are subscales
            ...
            for (subscaleIdx in 1:length(subscales)) {
                ...
                tempData <- dataLong[subscale == subscaleName, c(1, 2, 4)] %>% # subset subscale items, select pNo, item, and scoreR columns
            ...
            tempData <- dataLong[, c(1, 2, 4)] %>% # select pNo, item, and scoreR columns
                spread(item, scoreR) %>% # spread data to wide form
                select(-1) %>% # remove participant number column 
                as.data.frame() # psych::alpha function takes only dataframe
