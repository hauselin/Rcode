# Last modified by Hause Lin 09-03-18 00:02 hauselin@gmail.com
reportLM <- function(model, decimal = 2, showTable = FALSE, showEffectSizesTable = FALSE) {
    ...
    estimatesRound <- apply(estimatesRound, 2, function(x) ifelse(abs(x) < 0.01, sprintf(digits, x), sprintf(digits, x)))
    estimatesRound <- data.frame(term = estimates$term, estimatesRound)
    ...
    formattedOutputDf <- data_frame(term = as.character(estimates$term), 
    ...
    if (showTable) {
        ...
        estimatesOutput <- data.frame(term = as.character(estimates$term), estimatesOutput)
...
# reportLM(model3, showTable = F, showEffectSizesTable = T, decimal = 5)