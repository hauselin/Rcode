...
fit_ddmfull <- function(data, rts, responses, id = NULL, group = NULL, startParams = c(a = 2, v = 0.1, t0 = 0.3, z = 0.5, st0 = 0.1, sz = 0.1, sv = 0.1), simCheck = TRUE, decimal = 4, parallel = FALSE) {
    ...
    likelihood_ddm <- function(params, rt, response) {
        if (params['t0'] < 0.03 | params['a'] <= 0 | params['z'] <= 0 | params['z'] > 1 | params['st0'] < 0 | params['sz'] < 0 | params['sv'] < 0) return(1e6)
    ...
    resultsFinal <- dplyr::select(resultsWide2, id, group, n:n1, a, v, t0, z, st0, sz, sv, everything())
